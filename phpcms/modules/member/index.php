...
class index extends foreground {
	...
	public function logout() {
		...
		showmessage('退出成功'.$synlogoutstr, $forward);
	...
	//ajax登录
	public function public_checklogin_ajax() {
		...
		//判断验证码
		$code =isset($_POST['regcode']) && trim($_POST['regcode']) ? trim($_POST['regcode']) : exit('请输入验证码');
		if ($_SESSION['code'] != strtolower($code)) {
			$_SESSION['code'] = '';
			exit('验证码输入错误');
		}
		...
		$username = isset($_POST['username']) && is_username($_POST['username']) ? trim($_POST['username']) : exit('请输入您的手机号码');
		$password = isset($_POST['password']) && trim($_POST['password']) ? trim($_POST['password']) : exit('请输入您的登录密码');

		is_password($_POST['password']) && is_badword($_POST['password'])==false ? trim($_POST['password']) : exit('密码长度6-20位之间');
		$cookietime = intval($_POST['cookietime']);
		$synloginstr = ''; //同步登陆js代码
